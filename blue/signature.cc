...
static constexpr auto in_range(char x, char a, char b) -> char {
    return (x >= a && x <= b);
}

static constexpr auto get_bits(char x) -> char {
...
static constexpr auto get_byte(const char *x) -> u8 {
    return get_bits(x[0]) << 4 | get_bits(x[1]);
}

static auto find_pattern_internal(uptr start, uptr end, const char *pattern) -> u8 * {
    u8 *first = 0;
...
auto Signature::find_pattern(const char *pattern, uptr start, uptr length) -> u8 * {
    return find_pattern_internal(start, start + length, pattern);
}

auto Signature::resolve_callgate(void *address) -> void * {
